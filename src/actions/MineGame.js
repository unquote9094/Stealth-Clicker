    ...
    _registerAlertHandler() {
        ...
        this.page.on('dialog', async (dialog) => {
            ...
            try {
                ...
                // 이미 처리된 dialog는 무시 (MonsterRaid와 충돌 방지)
        ...
        try {
            ...
            const mines = await this.page.$$eval(MINE_CONFIG.SELECTORS.MINE_LIST.ITEM, (items) => {
                return items.map(item => {
                    ...
                    const isAlive = endDate.length === 0; // 단순! 종료일 없으면 살아있음
            ...
            // 광산 목록 로그 (debug 레벨 = 파일에만 저장)
            log.debug(`광산 목록: 총 ${mines.length}개, 살아있는 광산 ${aliveMines.length}개`);
            mines.slice(0, 3).forEach((m, i) => {
                log.debug(`  [${i}] ${m.name} | isAlive: ${m.isAlive} | endDate: "${m.endDate}"`);
            });
